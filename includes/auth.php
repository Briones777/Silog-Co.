...
/**
 * Notification delivery credentials (config/notifications.php).
 *
 * @return array<string, string>
 */
function notification_config(): array
{
    static $cfg = null;

    if ($cfg === null) {
        $cfg = require dirname(__DIR__) . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'notifications.php';
    }
 ...
 */
function normalize_e164(string $destination): string
{
    ...
    if ($destination[0] === '+') {
        return '+' . preg_replace('/\D/', '', substr($destination, 1));
    }
...
/**
 * Send the reset email through the Resend API.
 */
function send_email_via_resend(array $cfg, string $to, string $code): bool
{
    $escape = static fn (string $v): string => htmlspecialchars($v, ENT_QUOTES, 'UTF-8');

    $subject = 'Your Silog & Co. password reset code';
    $html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:480px;margin:0 auto;background:#fdf8ef;border:1px solid #e8ddc9;border-radius:12px;overflow:hidden">'
        . '<div style="background:#2b2118;padding:20px 24px"><span style="color:#f2b01e;font-size:12px;letter-spacing:2px;font-weight:bold">SILOG &amp; CO.</span>'
        . '<h1 style="color:#ffffff;margin:6px 0 0;font-size:22px;text-transform:uppercase">Password reset</h1></div>'
        . '<div style="padding:24px"><p style="color:#2b2118;font-size:14px;line-height:1.6;margin:0 0 16px">Use this 6-digit code to reset your password. It expires in '
        . RESET_CODE_TTL_MINUTES . ' minutes.</p>'
        . '<p style="text-align:center;margin:0 0 16px"><span style="display:inline-block;background:#ffffff;border:1px solid #e8ddc9;border-radius:8px;padding:12px 28px;font-size:28px;letter-spacing:8px;font-weight:bold;color:#d9281c">'
        . $escape($code) . '</span></p>'
        . '<p style="color:#7a6a58;font-size:12px;line-height:1.6;margin:0">Didn&#39;t request this? You can safely ignore the email — your password stays unchanged.</p></div></div>';
    $text = "Silog & Co. password reset code: {$code} (expires in " . RESET_CODE_TTL_MINUTES . " minutes). "
    ...
    if ($status < 200 || $status >= 300) {
        error_log("resend email failed (HTTP {$status}): " . substr((string)$body, 0, 300) . ' ' . $error);
...
/**
 * Send the reset SMS through the Vonage API.
 */
function send_sms_via_vonage(array $cfg, string $destination, string $code): bool
{
    $to = normalize_e164($destination);

    if ($to === '') {
        return false;
    }

    $text = "Silog & Co. password reset code: {$code} (valid " . RESET_CODE_TTL_MINUTES . ' min). '
    ...
    if (!isset($messages[0]['status']) || (string)$messages[0]['status'] !== '0') {
        error_log('vonage sms failed: ' . substr((string)$body, 0, 300) . ' ' . $error);
 ...
 *
 * Resend handles email, Vonage handles SMS — configure keys in
 * config/notifications.php (or the matching environment variables).
 * With no keys set, or when APP_DEBUG is on and delivery fails, the
 * code is mirrored into the session and shown on the reset page so
 * local testing never gets stuck. Returns true only on a real send.
 */
function deliver_reset_code(string $channel, string $destination, string $code): bool
{
    ...
    if ($channel === 'sms') {
        if ($cfg['vonage_api_key'] !== '' && $cfg['vonage_api_secret'] !== '') {
            $sent = send_sms_via_vonage($cfg, $destination, $code);
        } else {
            error_log("[notifications] Vonage not configured; SMS for {$destination} not sent");
        }
    } else {
        if ($cfg['resend_api_key'] !== '') {
            $sent = send_email_via_resend($cfg, $destination, $code);
        } else {
            error_log("[notifications] Resend not configured; email for {$destination} not sent");
        }
    }

    if (!$sent && APP_DEBUG) {
        $_SESSION['debug_reset_code'] = $code;
        error_log('[notifications] APP_DEBUG fallback: reset code mirrored to session');
    }
